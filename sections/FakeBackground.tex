...
Decays of the form \Bhh (where $h,h'$ are hadrons) are expected to form a peaking background at the $B$-meson mass when both the hadrons in the final state are misidentified as muons. %
...
For \Bhh, this is despite the anticipated $\mathcal{O}(10^{-3})$ probability for a hadron to be misidentified as a muon as, due to the large hadronic branching fractions of $B$-meson decays, the expected yield of \Bhh in the SR is comparable to that of \Bmumu. 
...
\begin{itemize}
    \item \textbf{Compute Faking Probabilities}: Calculate, from data (for kaons) or MC (for pions), the probability that a hadron fakes a muon.
    \item \textbf{Derive Functional Forms for \Bhh in the SR}: based upon applying various corrections to MC, including the faking probabilities from the previous step.
    \item \textbf{Estimate the yield of \Bhh in the SR}: both using a modified version of the master equation and verifying using data in control regions
    \item \textbf{Estimate the yield of Semi-leptonic backgrounds in the SR}: by using the yields in data sidebands and the faking probabilities. 
\end{itemize}
...
Once the faking probabilities are known, a functional form for \Bhh in the SR can be derived by applying these probabilities to MC samples of background processes that can contribute to a peaking fake background. 
