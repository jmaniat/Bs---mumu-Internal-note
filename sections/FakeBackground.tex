...
The faking probabilities of the pions are derived using the decay channel $B^+\to J/\psi(\to \mu^+\mu^-)\pi^+$ in MC, following an identical vertex-reconstruction and $B$-candidate selection approach as for $B^+\to J/\psi(\to \mu^+\mu^-)K^+$, except for relying on the identification of the pion using the MC-truth information. %
Now, two yields are extracted:
\begin{itemize}
    \item The yield of all of the $B^+$-candidates is obtained using an unbinned extended maximum likelihood fit to the reconstructed mass of the $B^+$-candidates. %
    This yield is our roundabout way of obtaining the number of positively charged pions, i.e., $N_{\pi^+}$. Recall that we are not really interested in the $B^+$-decay itself, but rather, we simply use this decay to tag the pions.
...
\subsubsection{Final \Bhh Functional From}
\label{subsubsec:BhhFunctionalForm}
\textcolor{red}{Dom: Add final Bhh shapes here.}
